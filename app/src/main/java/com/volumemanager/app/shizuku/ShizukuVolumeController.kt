...
class ShizukuVolumeController(
    ...
    private val playbackChangeListener = object : IPlaybackChangeListener.Stub() {
        override fun onPlaybackChanged() {
            mainHandler.post { notifyPlaybackChanged() }
        }
    }
    ...
    private val userServiceConnection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, binder: IBinder?) {
            ...
            try {
                ...
                svc.setPlaybackChangeListener(playbackChangeListener)
                ensureMediaStreamCeiling(svc)
            } catch (t: Throwable) {
    ...
    fun unbind() {
        try {
            service?.setPlaybackChangeListener(null)
    ...
    private fun ensureMediaStreamCeiling(svc: IVolumePrivilegedService) {
        ...
        val active = try {
            svc.listActivePlaybacks().map { it.packageName }
        } catch (_: Throwable) {
            emptyList()
        }
        MediaStreamCeiling.ensureMaxWithRescale(
            audioManager = am,
